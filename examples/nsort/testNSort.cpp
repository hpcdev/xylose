...
int main() {
    ...
    using namespace xylose::nsort;
    using namespace xylose::nsort::map;
    ...
    initPVector(pv,N_little, n_types);
    std::sort(pv.begin(), pv.end(), species_comp());
    ...
    if (n_types == 2) {
        initPVector(pv, N_little, n_types);
        std::partition(pv.begin(), pv.end(), partition_species_pred());
...
#define N_TIMERS 18
    ...
    for (int order = 1; order <= 100000; order *= 2) {
        ...
        for (int i = 0; i < N_TIMERS; i++) {
            timer[i].function = xylose::Timer::CUMMULATIVE;
            timer[i].zero();
        }
