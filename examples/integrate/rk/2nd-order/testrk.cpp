...
int main() {
  ...
  xylose::integrate::RK<GetDerivs, 2u> rk;
  ...
  while ( t <= T ) {
    ...
    Vector<double,6u> cal = p1, dx = dt * soln.d_dt(t);
  ...
  /* we try and match the error for the 5th-order adaptive routine. */
  double target_error = 0.28e-6L;

  std::cout << "Constant timestep used:  " << dt << std::endl;

  std::cout << "Number derivative evaluations:  " << rk.derivs.num_calls << "\n"
       "In units of the target error (" << target_error << ")\n"
       "                  X\t  Y\t  Z\t  VX\t  VY\t  VZ\n"
       "average error:  " << err_avg/target_error << "\n"
       "rms error    :  " << err_rms/target_error << "\n"
       "std(error)   :  " << err_sigma/target_error << std::endl;
