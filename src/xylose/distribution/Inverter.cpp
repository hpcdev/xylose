...
namespace xylose {
  namespace distribution {
    ...
    template < typename RNG >
    inline Inverter<RNG>::Inverter( const Inverter & that )
      : L(0), q(NULL), rng(that.rng) {
      copyLq(that.L, that.q);
    }


    template < typename RNG >
    inline Inverter<RNG>::Inverter( const int & _qLen,
                                    const double * _q,
                                    RNG & rng )
      : L(0), q(NULL), rng(rng) {
      copyLq(_qLen-1, q);
    ...
    inline Inverter<RNG>::Inverter( const DistroFunctor & distro,
                                    const double & min, const double & max,
                                    const int & nbins,
                                    RNG & rng )
      : L(nbins), q(NULL), rng(rng) {
