...
namespace xylose {
  namespace binning {
     ...
     * @see MultiValued for a keyed histogramming class.
     */
    ...
    public:
       ...
       *     application.
       */
      inline Extender(const double & mn  = 0.0, const double & mx  = 0.0,
      ...
      template <class Tf>
      inline Extender & operator*=(const Tf & factor) {
       ...
       * interface similar to MultiValued.
       */
       ...
       * This function will only compile if the child bin type has a bin()
       * interface similar to SingleValued.
       */
       ...
       */
      Extender & operator+=(const Extender & that) {
        ...
        typedef struct {
           ...
           * @see Extender::operator+=(const Extender &).
           */
          static void MPISUM(const Extender* in, Extender* inout, int len, const MPI::Datatype & typ) {
            for (int i = 0; i < len; ++i)
              inout[i] += in[i];
          }
    ...
    #ifdef USE_MPI
      template <class TKey, class T2, unsigned int nbins>
      typename Extender<TKey,T2,nbins>::MPI_INFO Extender<TKey,T2,nbins>::MPI = {
        MPI::OP_NULL,       /* SUM */
      ...
      template <class TKey, class T2, unsigned int nbins>
      int Extender<TKey,T2,nbins>::added_mpi_init = Extender<TKey,T2,nbins>::MPI.add_init();



      /** blockAdder for Extender. 
       * reference layout:
          struct Extender {
            double max;
            double min;
      ...
      */
      ...
      void blockAdder( std::vector<Block> & blocks,
                     const Extender<TKey,T2,nbins> * null_ptr ) {
        ...
        int pad = sizeof(Extender<TKey,T2,nbins>)
