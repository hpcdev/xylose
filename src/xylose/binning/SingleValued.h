...
/** \example binning/single/testbin.cpp
 * Demonstrates the simple binning utility class. 
 * @see SingleValued
 */


#ifndef xylose_binning_SingleValued_h
#define xylose_binning_SingleValued_h
...
namespace xylose {
  namespace binning {
    ...
    public:
       ...
       */
      inline SingleValued(const double & mn = 0.0, const double & mx = 0.0) {
        init(mn,mx);
      ...
      inline void bin(const TKey & key, const TBin & increment = TBin(1) ) {
        getBin(key) += increment;
      }
      ...
      template <class Tf>
      inline SingleValued & operator*=(const Tf & factor) {
       ...
       */
      SingleValued & operator+=(const SingleValued & that) {
        ...
        typedef struct {
           ...
           * @see SingleValued::operator+=(const SingleValued &).
           */
          static void MPISUM(const SingleValued* in, SingleValued* inout, int len, const MPI::Datatype & typ) {
            for (int i = 0; i < len; i++) {
              inout[i] += in[i];
            }
          }
      ...
      typename SingleValued<TKey,nbins,TBin>::MPI_INFO SingleValued<TKey,nbins,TBin>::MPI = {
        MPI::OP_NULL,       /* SUM */
      ...
      template <class TKey, unsigned int nbins, class TBin>
      int SingleValued<TKey,nbins,TBin>::added_mpi_init = SingleValued<TKey,nbins,TBin>::MPI.add_init();



      /** blockAdder for SingleValued<Tkey,nbins,TBin = double>. 
       * reference layout:
          struct SingleValued {
            double max;
            double min;
      ...
      */
      ...
      void blockAdder( std::vector<Block> & blocks,
                       const SingleValued<TKey,nbins,double> * null_ptr ) {
      ...
      /** blockAdder for SingleValued<Tkey,nbins,TBin = Vector<double,L> >. 
       * reference layout:
          struct SingleValued {
            double max;
            double min;
      ...
      */
      ...
      void blockAdder( std::vector<Block> & blocks,
                       const SingleValued<TKey,nbins,Vector<double,L> > * null_ptr ) {
        blocks.push_back(Block(3,sizeof(double))); /* pad */
        blocks.push_back(Block(L*nbins,sizeof(double),MPI::DOUBLE));
        /* shouldn't have to worry about alignment issues for doubles (yet) */
      }

      /** blockAdder for SingleValued<Tkey,nbins,TBin = float>. 
       * reference layout:
          struct SingleValued {
            double max;
            double min;
      ...
      */
      template <class TKey, unsigned int nbins, class Block, unsigned int L>
      void blockAdder( std::vector<Block> & blocks,
                       const SingleValued<TKey,nbins,Vector<float,L> > * null_ptr ) {
        blocks.push_back(Block(3,sizeof(double))); /* pad */
        blocks.push_back(Block(L*nbins,sizeof(float),MPI::FLOAT));

        /* now we'll add any remaining padding necessary--helpful for arrays */
        int pad = sizeof(SingleValued<TKey,nbins,Vector<float,L> >)
                - (3*sizeof(double) + L*nbins*sizeof(Vector<float,L>));
        if (pad) blocks.push_back(Block(1,pad));    /* pad */
      }

      /** blockAdder for SingleValued<Tkey,nbins,TBin = int>. 
       * reference layout:
          struct SingleValued {
            double max;
            double min;
      ...
      */
      template <class TKey, unsigned int nbins, class Block>
      void blockAdder( std::vector<Block> & blocks,
                       const SingleValued<TKey,nbins,int> * null_ptr ) {
        blocks.push_back(Block(3,sizeof(double))); /* pad */
        blocks.push_back(Block(nbins,sizeof(int),MPI::INT));

        /* now we'll add any remaining padding necessary--helpful for arrays */
        int pad = sizeof(SingleValued<TKey,nbins,int>)
                - (3*sizeof(double) + nbins*sizeof(int));
        if (pad) blocks.push_back(Block(1,pad));    /* pad */
      }

      /** blockAdder for SingleValued<Tkey,nbins,TBin = Vector<int,L> >. 
       * reference layout:
          struct SingleValued {
            double max;
            double min;
      ...
      */
      template <class TKey, unsigned int nbins, class Block, unsigned int L>
      void blockAdder( std::vector<Block> & blocks,
                       const SingleValued<TKey,nbins,Vector<int,L> > * null_ptr ) {
        blocks.push_back(Block(3,sizeof(double))); /* pad */
        blocks.push_back(Block(L*nbins,sizeof(int),MPI::INT));

        /* now we'll add any remaining padding necessary--helpful for arrays */
        int pad = sizeof(SingleValued<TKey,nbins,Vector<int,L> >)
...
#endif // xylose_binning_SingleValued_h
